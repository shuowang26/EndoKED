...
def test(model, path, dataset,writer,epoch):
...
def train(train_loader, model, optimizer, epoch, test_path,tb_path):
    model.train()
    global best
    ...
    if (epoch + 1) % 1 == 0:
        for dataset in ['CVC-300', 'CVC-ClinicDB', 'Kvasir', 'CVC-ColonDB', 'ETIS-LaribPolypDB','test']:
            dataset_dice = test(model, test_path, dataset,writer,epoch)
            if opt.local_rank == 0:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--train_save', type=str,
                        default='./model_CAM_pth/'+model_name+'/')
    parser.add_argument("--local_rank", default=-1, type=int, help="local_rank")
    parser.add_argument("--log_file_name", default='./log/all_train_log_CAM.log', type=str, help="all_train_log.log")
    ...
    tb_path = f'{log_root}/tensorboard_log_all_from_{type}'
    os.makedirs(log_root,exist_ok=True)
    os.makedirs(tb_path,exist_ok=True)
    os.makedirs(opt.train_save,exist_ok=True)

    img_path = f'/data/PROJECTS/Endo_GPT/EndoGPT_WSSS/datasets/a_zhongshan_2wpublic_all/filtered_after_{type}_zhongshan_2wpublic_imgPath.csv'
    label_root_path = f'/data/PROJECTS/Endo_GPT/EndoGPT_WSSS/0623_{type}_seeds/'
    test_path = '/data/Datasets/MedicalDatasets/息肉数据集/息肉公开数据集/dataset/TestDataset/'
    img_path_list, label_lst = [], []
    ...
    with open (img_path,'r') as f:
        ...
        for item in reader:
            ...
            label_lst.append(label_path)

    img_path_list = glob('/data/Datasets/MedicalDatasets/息肉数据集/息肉公开数据集/dataset/TrainDataset/images/*')
    label_lst = glob('/data/Datasets/MedicalDatasets/息肉数据集/息肉公开数据集/dataset/TrainDataset/masks/*')
    train_loader = get_loader(img_path_list, label_lst, batchsize=opt.batchsize, trainsize=opt.trainsize,
    ...
    for epoch in range(1, opt.epoch):
        ...
        train(train_loader, model, optimizer, epoch, test_path,tb_path)
