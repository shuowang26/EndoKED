...
if __name__ == '__main__':
    ...
    parser.add_argument('--pt_path', type=str, default='./EndoKED_SEG_Baselines/LDNet-main/logs/Train_on_KvasirandDB/2024-04-21-05-33-40_loaded_from_endoked/Train_on_KvasirandDB_Best.pt')
    parser.add_argument('--testdata_path', type=str, default='./data/polyp_public/TestDataset/', help='testing size')
    parser.add_argument('--model', type=str, default='LDNet')
    parser.add_argument('--exp_tag', type=str, default='test_debug', help='testing size')
    ...
    for _data_name in dataset_lst:
        ...
        items = sorted(glob(f"{gt_root}/*"))

        num1 = len(os.listdir(gt_root))
        valid_data = getattr(datasets, "PolygenDataset")(opt, data_path, mode='valid')
        test_loader = DataLoader(valid_data, batch_size=1, shuffle=False, num_workers=8)
        DSC = 0.0
        JACARD = 0.0
        preds = []
        gts = []
        ...
        for i, data in tqdm(enumerate(test_loader)):
            image, gt = data['image'], data['label']
            name = os.path.basename(items[i])
