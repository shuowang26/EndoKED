...
parser.add_argument("--model_path", default="/data/PROJECTS/Endo_GPT/EndoGPT_WSSS/work_dir_voc_wseg/SOTA_Segmentation_0613_0.783dice_20000th/checkpoints/model_iter_20000.pth", type=str, help="model_path")
...
def crf_proc(name_lst,dataset_type):
    print("crf post-processing...")

    name_list = name_lst

    #### CLC_DB
    if dataset_type == 'ClinicDB':
        ...
        images_path = '/data/Datasets/MedicalDatasets/息肉数据集/息肉公开数据集/dataset/TestDataset/CVC-ClinicDB/images'
        labels_path = '/data/Datasets/MedicalDatasets/息肉数据集/息肉公开数据集/dataset/TestDataset/CVC-ClinicDB/masks'
    #### Kvasir-SEG
    elif dataset_type == 'Kvasir-SEG':
        images_path = '/data/Datasets/MedicalDatasets/息肉数据集/息肉公开数据集/Kvasir-SEG/images/'
        labels_path = "/data/Datasets/MedicalDatasets/息肉数据集/息肉公开数据集/Kvasir-SEG/masks/"
    ...
    def _job(i):
        ...
        ## Kvasir-SEG
            ...
            image = imageio.imread(image_name).astype(np.float32)
            label_name = os.path.join(labels_path, name + ".jpg")
...
def validate(args=None,dataset_type='ClinicDB'):
    ...
    ### Kvasir-SEG
        ...
        root_dir_test = "/data/Datasets/MedicalDatasets/息肉数据集/息肉公开数据集/Kvasir-SEG/"
        # root_dir_test = '/data/Datasets/MedicalDatasets/息肉数据集/息肉公开数据集/dataset/TestDataset/Kvasir/'

    # root_dir_test = '/data/PROJECTS/Endo_GPT/Datasets/test'
    img_path_list_test, mask_test_path = load_test_img_mask(root_dir_test,dataset_type=dataset_type)
    ...
    crf_score = crf_proc(name_lst,dataset_type)
...
if __name__ == "__main__":
    ...
    dataset_type = 'Kvasir-SEG' #'ClinicDB'
    # dataset_type = 'ClinicDB'
    base_dir = args.model_path.split("checkpoints")[0]
    args.logits_dir = os.path.join(base_dir, f"segs_s{dataset_type}/logits", args.infer_set)
    args.segs_dir = os.path.join(base_dir, f"segs_s{dataset_type}/seg_preds", args.infer_set)
    args.segs_rgb_dir = os.path.join(base_dir, f"segs_s{dataset_type}/seg_preds_rgb", args.infer_set)
    ...
    print(args)
    validate(args=args,dataset_type=dataset_type)
