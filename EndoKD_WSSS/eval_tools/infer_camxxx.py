...
from datasets import dataset_loader_public_data as voc
from datasets.dataset_loader_public_data import load_img_label_info_from_csv, load_test_img_mask,load_img_label_info_from_public_data
...
parser.add_argument("--model_path", default="/data/PROJECTS/Endo_GPT/EndoGPT_WSSS/work_dir_voc_wseg/SOTA_0508_0.61dice/checkpoints/model_iter_18000.pth", type=str, help="model_path")
...
def _validate(model=None, data_loader=None, args=None):
    ...
    base_dir = '/data/PROJECTS/Endo_GPT/EndoGPT_WSSS/ear1521_CAM'
    ...
    with torch.no_grad(), torch.cuda.device(0):
        ...
        for idx, data in tqdm(enumerate(data_loader), total=len(data_loader), ncols=100, ascii=" >="):
            ...
            inputs = inputs.cuda()
            img = cv2.imread('/home/jaye/Downloads/EAE/1521.jpg').astype(np.float64)
            # img = np.asarray(imageio.imread('/home/jaye/Downloads/1211042-a1-G4-N1S_Mutation.png')).astype(np.float64)
            img_ = torch.tensor(img.transpose(2,0,1)[None,...])
            img = F.interpolate(img_, size=labels.shape[1:], mode='bilinear', align_corners=False)
...
def validate(args=None):
    ...
    root_dir_test = "/data/Datasets/MedicalDatasets/Endotest_set/dataset/CVC-ClinicDB_PNG_datasets/"
