...
from datasets.dataset_loader_zhongshan_Large import load_img_label_info_from_csv, load_test_img_mask
from datasets import dataset_loader_zhongshan_Large as voc
...
def _validate(model=None, data_loader=None, args=None):
    ...
    base_dir = '/data/PROJECTS/Endo_GPT/EndoGPT_WSSS/visualize_paper'
    ...
    with torch.no_grad(), torch.cuda.device(0):
        ...
        for idx, data in tqdm(enumerate(data_loader), total=len(data_loader), ncols=100, ascii=" >="):
            ...
            imageio.imsave(os.path.join(cam_dir, name[0] + "_LABEL.jpg"), imutils.encode_cmap(cam_label[0].cpu().numpy().astype(np.uint8)))
            imageio.imsave(os.path.join(cam_aux_dir, name[0] +"_LABEL.jpg"), imutils.encode_cmap(cam_aux_label[0].cpu().numpy().astype(np.uint8)))
