...
def tensorboard_image(imgs=None, cam=None,nrow=4):
    ## images

    _imgs = denormalize_img(imgs=imgs)
    grid_imgs = torchvision.utils.make_grid(tensor=_imgs, nrow=nrow)

    cam = F.interpolate(cam, size=_imgs.shape[2:], mode='bilinear', align_corners=False)
    cam = cam.cpu()
    ########################
    # cam_ = torch.zeros_like(cam).cpu()
    # # cam_[:,11,:,:]  = cam[:,11,:,:]
    # cam_[:,7,:,:] = cam[:,7,:,:]
    # cam = cam_
    ...
    grid_cam = torchvision.utils.make_grid(tensor=cam_img.type(torch.uint8), nrow=nrow)

    return grid_imgs, grid_cam

def tensorboard_image_only(imgs=None, nrow=4):
    ## images
...
def tensorboard_image_solo(imgs=None, cam=None,):
    ## images
    ...
    cam_max = cam.max(dim=1)[0]
    cam_heatmap = plt.get_cmap('jet')(cam_max.numpy())[:,:,:,0:3]*255
    cam_cmap = torch.from_numpy(cam_heatmap)[0].permute([2,0,1])
    cam_img = cam_cmap*0.5 + _imgs[0].cpu()*0.5
    grid_cam = torchvision.utils.make_grid(tensor=cam_img.type(torch.uint8), nrow=2)

    return _imgs[0],cam_img
...
def tensorboard_label_solo(labels=None):
    ## labels
    labels_cmap = encode_cmap(np.squeeze(labels))
    labels_cmap = torch.from_numpy(labels_cmap).permute([2, 0, 1])
    grid_labels = torchvision.utils.make_grid(tensor=labels_cmap,)
