...
def img_resize_short_zhongshan(image, min_size=512):
    h, w, _ = image.shape
    # if min(h, w) >= min_size:
    #     return image
    ...
    new_image = Image.fromarray(image.astype(np.uint8)).resize(new_scale, resample=Image.BILINEAR)
    new_image = np.asarray(new_image).astype(np.uint8)
...
def random_crop_with_pseudo_labels_sam(image, label=None, crop_size=None, mean_rgb=[0,0,0], ignore_index=255):
    ...
    pad_label = np.ones((H,W), dtype=np.uint8) * 0
    pad_label[H_pad:(H_pad+h), W_pad:(W_pad+w)] = label
    label = pad_label[H_start:H_end, W_start:W_end]
