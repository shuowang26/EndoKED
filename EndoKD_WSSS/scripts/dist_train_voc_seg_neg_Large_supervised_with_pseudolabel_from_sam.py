...
from datasets import dataset_loader_zhongshan_Large_CAM_supervised_with_pseudolabel_fromSAM as voc
from datasets.dataset_loader_zhongshan_Large_CAM_supervised_with_pseudolabel_fromSAM import load_img_label_info_from_csv, load_test_img_mask
...
def train(args=None):
    ...
    for n_iter in range(args.max_iters):
        ...
        try:
            inputs, cls_label, seg_mask, img_box, crops = next(train_loader_iter)
        ...
        pseudo_label_aux = F.interpolate((seg_mask[:,None,...]), size=fmap.shape[2:], mode="bilinear", align_corners=False)

        aff_mask = label_to_aff_mask(pseudo_label_aux[:,0,...])
        ...
        if (n_iter + 1) % 20 == 0:
            ...
            grid_masks = imutils2.tensorboard_label(labels=seg_mask.cpu().numpy().astype(np.int16),nrow=4)
            ...
            #writer.add_image("train/pseudo_irn_gts", grid_irn_gt, global_step=n_iter)
            writer.add_image("cam/pseudo_gt", grid_masks, global_step=n_iter)
            writer.add_image("cam/valid_cams", grid_cam, global_step=n_iter)
            writer.add_image("cam/aux_cams", grid_cam_aux, global_step=n_iter)

        writer.add_scalars('train/loss', {"seg_loss": seg_loss.item(),"dice_loss": dice_loss.item(), "cls_loss": cls_loss.item(),"total_loss": loss.item(),"reg_loss": reg_loss.item(),"ptc_loss": ptc_loss.item(),"ctc_loss": ctc_loss.item(),}, global_step=n_iter)
